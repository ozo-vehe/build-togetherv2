...
<template>
  <section class="flex flex-wrap flex-col gap-4 py-20 justify-center items-center">
    ...
    <div class="mt-16 flex flex-wrap items-start justify-center w-full gap-x-32 gap-y-4">
            ...
            <svg xmlns="http://www.w3.org/2000/svg" width="33" height="33" viewBox="0 0 33 33" fill="none">
              ...
              <defs>
                <clipPath id="clip0_181_161">
                  <rect width="32" height="32" fill="white" transform="translate(0.514404 0.769775)" />
                </clipPath>
              </defs>
            </svg>
          </RouterLink>
      ...
      <form @submit="handleSubmit" class="w-450 flex flex-col gap-8">
        <div class="form_input">
          <label for="name" class="">Name</label>
          <input v-model="form.name" class="" type="text" name="name" id="name" placeholder="Full name" required>
        </div>

        <div class="form_input">
          <label for="email">Email</label>
          <input v-model="form.email" type="email" name="email" id="email" placeholder="Email" required>
        </div>

        <div class="form_input">
          <label for="phone_number">Phone Number</label>
          <input v-model="form.phone_number" type="tel" name="phone_number" id="phone_number" placeholder="Phone Number">
        </div>

        <div class="form_input">
          <label for="message">Message</label>
          <textarea v-model="form.message" class="" name="message" id="message" placeholder="Message" required></textarea>
        </div>

        <button type="submit" class="w-40 px-5 py-3 rounded-xl bg-custom_orange text-slate-50">Send</button>
      </form>
    </div>
  </section>
</template>
